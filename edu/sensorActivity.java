...
public class sensorActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Switch sens_main=(Switch) findViewById(R.id.switch_sensmf);
        final Switch sens_up=(Switch) findViewById(R.id.switch_sensup);

        sens_main.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if(result.equals("true\n"))
                    Toast.makeText(getBaseContext(),"Sensors are " + status1,Toast.LENGTH_SHORT).show();
            }
        });
        sens_up.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(isChecked)
                {
                ...
                if(result.equals("true\n"))
                    Toast.makeText(getBaseContext(),"Sensors are " + status2 ,Toast.LENGTH_SHORT).show();
            }
        });
    ...
    public String getConnection1(String status1,String status2){
        ...
        try{
            ...
            HttpPost httppost = new HttpPost("http://192.168.1.3/sensormain.php");
    ...
    public String getConnection2(String status1,String status2){
        ...
        try{
            ...
            // have to change the ip here to correct ip
            HttpPost httppost = new HttpPost("http://192.168.1.3/sensorup.php");
            httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs1));
            HttpResponse response = httpclient.execute(httppost);
            HttpEntity entity = response.getEntity();
            inputStream = entity.getContent();
