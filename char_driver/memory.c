...
ssize_t retval = 0; 
 	if(count == 0)
  		return 0;
 	while(ioread32(UART_FR(addrv)) & UART_FR_RXFE);
 		val = ioread8(UART_DR(addrv));
	...
	{
  		if(ioread32(UART_FR(addrv)) & UART_FR_RXFE) {
   			break;
 		}
 		val = ioread8(UART_DR(addrv));
